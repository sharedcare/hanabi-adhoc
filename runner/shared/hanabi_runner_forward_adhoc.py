...
class HanabiRunner(Runner):
    ...
    @torch.no_grad()
    def eval_100k(self, eval_games=400):
        eval_envs = self.eval_envs
        trials = int(eval_games/self.n_eval_rollout_threads)

        # self.other_agent = AdvancedSimpleAgent({'players': 2})
        self.other_agent = RainbowAgent({'players': 2, 'num_moves': 20, 'observation_size': 658, 'checkpoint_dir': "/home/tony/Projects/hanabi/on-policy/onpolicy/scripts/eval/rainbow_models/rainbow-model-1"})
        ...
        for trial in range(trials):
            ...
            eval_obs, eval_share_obs, eval_available_actions, observations = eval_envs.reset_raw(eval_reset_choose)
            ...
            while True:
                ...
                for agent_id in range(self.num_agents):
                    ...
                    # Obser reward and next obs
                    eval_obs, eval_share_obs, eval_rewards, eval_dones, eval_infos, eval_available_actions, observations = eval_envs.step_raw(eval_actions)
