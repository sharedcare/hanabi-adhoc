...
class SharedReplayBuffer(object):
    ...
    def meta_recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        """
        Yield training data for chunked RNN meta training.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        :param data_chunk_length: (int) length of sequence chunks with which to train RNN.
        """
        ...
        if len(self.share_obs.shape) > 4:
            ...
            obs = self.obs[:-1].transpose(1, 2, 0, 3,
                                          4, 5).reshape(-1, *self.obs.shape[3:])
        else:
            share_obs = _cast(self.share_obs[:-1])
            obs = _cast(self.obs[:-1])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                ind = index * data_chunk_length
                ind_prev = (index - 1) * data_chunk_length if index > 0 else ind
                # size [T+1 N M Dim]-->[T N M Dim]-->[N,M,T,Dim]-->[N*M*T,Dim]-->[L,Dim]
                share_obs_batch.append(share_obs[ind:ind + data_chunk_length])
                obs_batch.append(obs[ind:ind + data_chunk_length])
                actions_batch.append(actions[ind:ind + data_chunk_length])
                ...
                prev_rewards_batch.append(rewards[ind_prev:ind_prev + data_chunk_length])
                prev_actions_batch.append(actions[ind_prev:ind_prev + data_chunk_length])
            ...
            # Meta params
            prev_rewards_batch = np.stack(prev_rewards_batch)
            ...
            # Meta params
            prev_rewards_batch = _flatten(L, N, prev_rewards_batch)
            prev_actions_batch = _flatten(L, N, prev_actions_batch)

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                adv_targ, available_actions_batch, prev_rewards_batch, prev_actions_batch
